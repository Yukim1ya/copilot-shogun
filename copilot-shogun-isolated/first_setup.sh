...
if command -v claude &> /dev/null; then
    # copilot コマンドは存在する → 実際に動くかチェック
    # COPILOT_VERSION=$(copilot --version 2>&1)
    # COPILOT_PATH=$(which copilot 2>/dev/null)

    if [ $? -eq 0 ] && [ "$CLAUDE_VERSION" != "unknown" ] && [[ "$CLAUDE_VERSION" != *"not found"* ]]; then
        # 動作する copilot が見つかった → バージョン判定
...
skill:
  ...
  # Copilot CLI専用化: claude用パス記述を完全削除（全処理削除済み）
...
# Copilot CLI専用化: Memory MCP セットアップ処理は不要のため削除
...
echo "     3. /exit で退出"
echo ""
echo "     ※ 一度認証すれば ~/.config/copilot/ に保存され、以降は不要です"
echo ""
echo "  ────────────────────────────────────────────────────────────────"
echo ""
echo "  出陣（全エージェント起動）:"
